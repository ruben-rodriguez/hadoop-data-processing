...
df_load.write.csv("hdfs://hadoop-master:9000/dataProcessing/output/destination-locations.csv")

df_load.groupBy('origin').count().select('origin', f.col('count').alias('count')).show()
df_load.write.csv("hdfs://hadoop-master:9000/dataProcessing/output/origin-locations.csv")


df_load.groupBy('destination', 'vehicle_type').count().select('destination', f.col('count').alias('count')).show()
df_load.write.csv("hdfs://hadoop-master:9000/dataProcessing/output/destination-vehicleTypes.csv")


df_load.groupBy('origin', 'vehicle_type').count().select('origin', f.col('count').alias('count')).show()
df_load.write.csv("hdfs://hadoop-master:9000/dataProcessing/output/origin-vehicleTypes.csv")
